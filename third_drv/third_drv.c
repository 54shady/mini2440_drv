...
{
	if (request_irq(IRQ_EINT8,  third_irq_handle, IRQT_BOTHEDGE, "k1", &pindesc[0]))
		return -EAGAIN;
	if (request_irq(IRQ_EINT11, third_irq_handle, IRQT_BOTHEDGE, "k2", &pindesc[1]))
		return -EAGAIN;
	if (request_irq(IRQ_EINT13, third_irq_handle, IRQT_BOTHEDGE, "k3", &pindesc[2]))
		return -EAGAIN;
	if (request_irq(IRQ_EINT14, third_irq_handle, IRQT_BOTHEDGE, "k4", &pindesc[3]))
		return -EAGAIN;
	if (request_irq(IRQ_EINT15, third_irq_handle, IRQT_BOTHEDGE, "k5", &pindesc[4]))
		return -EAGAIN;
	if (request_irq(IRQ_EINT19, third_irq_handle, IRQT_BOTHEDGE, "k6", &pindesc[5]))
		return -EAGAIN;
...
{
	unregister_chrdev(major, "third_dev");
