...
static int fifth_drv_open(struct inode *inode, struct file *file)
{
	 ...
	 * EINT19--GPG11--k6-->63
	 */
	if (request_irq(IRQ_EINT8,  fifth_irq_handle, IRQT_BOTHEDGE, "k1", &pindesc[0]))
		return -EAGAIN;
	if (request_irq(IRQ_EINT11, fifth_irq_handle, IRQT_BOTHEDGE, "k2", &pindesc[1]))
		return -EAGAIN;
	if (request_irq(IRQ_EINT13, fifth_irq_handle, IRQT_BOTHEDGE, "k3", &pindesc[2]))
		return -EAGAIN;
	if (request_irq(IRQ_EINT14, fifth_irq_handle, IRQT_BOTHEDGE, "k4", &pindesc[3]))
		return -EAGAIN;
	if (request_irq(IRQ_EINT15, fifth_irq_handle, IRQT_BOTHEDGE, "k5", &pindesc[4]))
		return -EAGAIN;
	if (request_irq(IRQ_EINT19, fifth_irq_handle, IRQT_BOTHEDGE, "k6", &pindesc[5]))
		return -EAGAIN;
